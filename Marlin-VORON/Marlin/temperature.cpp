...
static volatile bool temp_meas_ready = false;
  ...
  static float temp_iState[HEATER_BLOCKS] = { 0 };
  static float temp_dState[HEATER_BLOCKS] = { 0 };
  static float pTerm[HEATER_BLOCKS];
  static float iTerm[HEATER_BLOCKS];
  static float dTerm[HEATER_BLOCKS];
  #if ENABLED(PID_ADD_EXTRUSION_RATE)
    static float cTerm[HEATER_BLOCKS];
    static long last_position[HEATER_BLOCKS];
    static long lpq[LPQ_MAX_LEN];
    static int lpq_ptr = 0;
  #endif
  //int output;
  static float pid_error[HEATER_BLOCKS];
  static float temp_iState_min[HEATER_BLOCKS];
  static float temp_iState_max[HEATER_BLOCKS];
  static bool pid_reset[HEATER_BLOCKS];
#endif //PIDTEMP
#if ENABLED(PIDTEMPBED)
  //static cannot be external:
...
#endif //PIDTEMPBED
static unsigned char soft_pwm[HEATER_BLOCKS];
...
static int minttemp_raw[HEATER_BLOCKS] = ARRAY_BY_HEATER_BLOCKS(HEATER_0_RAW_LO_TEMP , HEATER_1_RAW_LO_TEMP , HEATER_2_RAW_LO_TEMP, HEATER_3_RAW_LO_TEMP);
static int maxttemp_raw[HEATER_BLOCKS] = ARRAY_BY_HEATER_BLOCKS(HEATER_0_RAW_HI_TEMP , HEATER_1_RAW_HI_TEMP , HEATER_2_RAW_HI_TEMP, HEATER_3_RAW_HI_TEMP);
static int minttemp[HEATER_BLOCKS] = { 0 };
static int maxttemp[HEATER_BLOCKS] = ARRAY_BY_HEATER_BLOCKS1(16383);
#ifdef BED_MINTEMP
...
#else
  static void* heater_ttbl_map[HEATER_BLOCKS] = ARRAY_BY_HEATER_BLOCKS((void*)HEATER_0_TEMPTABLE, (void*)HEATER_1_TEMPTABLE, (void*)HEATER_2_TEMPTABLE, (void*)HEATER_3_TEMPTABLE);
  static uint8_t heater_ttbllen_map[HEATER_BLOCKS] = ARRAY_BY_HEATER_BLOCKS(HEATER_0_TEMPTABLE_LEN, HEATER_1_TEMPTABLE_LEN, HEATER_2_TEMPTABLE_LEN, HEATER_3_TEMPTABLE_LEN);
#endif
...
#if ENABLED(THERMAL_PROTECTION_HOTENDS)
  int watch_target_temp[HEATER_BLOCKS] = { 0 };
  millis_t watch_heater_next_ms[HEATER_BLOCKS] = { 0 };
#endif
...
static void updateTemperaturesFromRawValues() {
  ...
  #endif
  for (uint8_t e = 0; e < HEATER_BLOCKS; e++) {
...
void tp_init() {
    ...
    TEMP_MAX_ROUTINE(0);
      ...
      TEMP_MAX_ROUTINE(1);
        ...
        TEMP_MAX_ROUTINE(2);
          ...
          TEMP_MAX_ROUTINE(3);
        #endif
      #endif // HEATER_BLOCKS > 3
    #endif // HEATER_BLOCKS > 2
  #endif // HEATER_BLOCKS > 1
  ...
  void thermal_runaway_protection(TRState* state, millis_t* timer, float temperature, float target_temperature, int heater_id, int period_seconds, int hysteresis_degc) {
    ...
    static float tr_target_temperature[HEATER_BLOCKS + 1] = { 0.0 };
    ...
    */
    ...
    int heater_index = heater_id >= 0 ? heater_id : HEATER_BLOCKS;
...
void disable_all_heaters() {
  for (int i = 0; i < HEATER_BLOCKS; i++) setTargetHotend(0, i);
  setTargetBed(0);
  ...
  #if HEATER_BLOCKS > 1 && HAS_TEMP_1
    DISABLE_HEATER(1);
  #endif

  #if HEATER_BLOCKS > 2 && HAS_TEMP_2
    DISABLE_HEATER(2);
  #endif

  #if HEATER_BLOCKS > 3 && HAS_TEMP_3
    DISABLE_HEATER(3);
  #endif
...
ISR(TIMER0_COMPB_vect) {
  ...
  ISR_STATICS(0);
    ...
    if ((pwm_count % 64) == 0) {
      ...
      #if HEATER_BLOCKS > 1    // EXTRUDER 1
        if (state_timer_heater_1 > 0) state_timer_heater_1--;
        #if HEATER_BLOCKS > 2    // EXTRUDER 2
          if (state_timer_heater_2 > 0) state_timer_heater_2--;
          #if HEATER_BLOCKS > 3    // EXTRUDER 3
